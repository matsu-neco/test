...
function setupItemEvents(div) {
    ...
    // ダブルクリックで名前（スマホはこれに代わる手段が今後必要かも）
...
function loadLayout() {
    ...
    items.forEach(item => {
        ...
        // 比率からピクセルに復元
        const leftPx = item.leftRatio ? (item.leftRatio * stage.offsetWidth) : parseInt(item.left);
        const topPx = item.topRatio ? (item.topRatio * stage.offsetHeight) : parseInt(item.top);
...
function exportImage() {
    ...
    img.onload = () => {
        ...
        link.href = canvas.toVDataURL ? canvas.toVDataURL("image/png") : canvas.toDataURL("image/png");
        link.click();
    };
...
Object.keys(INSTRUMENTS).forEach(key => {
    ...
    const info = INSTRUMENTS[key];
    btn.innerHTML = `<span class="btn-icon">${info.icon}</span><span>${info.btnLabel}</span>`;
